...
-- /// Logo asset loader (uses getcustomasset when available, else falls back to rbxassetid) ///
function Custom:LoadLogoAsset(url, fileName)
  local ok, data = pcall(function() return game:HttpGet(url) end)
  if ok and data and typeof(writefile) == "function" and typeof(getcustomasset) == "function" then
    local okW = pcall(writefile, fileName, data)
    if okW then
      local okA, asset = pcall(getcustomasset, fileName)
      if okA and asset then return asset end
    end
  end
  return "rbxassetid://136890595976124"
end

Custom.LogoAsset = Custom:LoadLogoAsset(
  "https://i.ibb.co/ZpCvx0Xf/06fa051d4123b6184cadfc4fa5821f5a.jpg",
  "SpeedHubX_Logo.jpg"
)
...
local function OpenClose()
  ...
  Close_ImageButton.MouseEnter:Connect(function()
    TweenService:Create(Close_ImageButton, TweenInfo.new(0.18), {ImageTransparency = 0.15, Size = UDim2.new(0, 66, 0, 66)}):Play()
  end)
  Close_ImageButton.MouseLeave:Connect(function()
    TweenService:Create(Close_ImageButton, TweenInfo.new(0.22), {ImageTransparency = 0, Size = UDim2.new(0, 62, 0, 62)}):Play()
  end)
...
function Speed_Library:CreateWindow(Config)
  ...
  -- /// Responsive: auto-scale on small/mobile screens ///
  local UIScaler = Custom:Create("UIScale", { Scale = 1 }, Main)
  local function ApplyResponsiveScale()
    local cam = workspace.CurrentCamera
    if not cam then return end
    local vp = cam.ViewportSize
    local designW, designH = SizeUi.X.Offset + 60, SizeUi.Y.Offset + 80
    local sx = vp.X / designW
    local sy = vp.Y / designH
    UIScaler.Scale = math.clamp(math.min(sx, sy), 0.5, 1)
    -- re-center the window in the viewport after rescale
    local sW = SizeUi.X.Offset * UIScaler.Scale
    local sH = SizeUi.Y.Offset * UIScaler.Scale
    DropShadowHolder.Position = UDim2.new(0, (vp.X // 2) - (sW // 2), 0, (vp.Y // 2) - (sH // 2))
  end
  ...
  -- /// Improved top-bar gradient: subtle red bleed under the title ///
  Custom:Create("UIGradient", {
    Rotation = 90,
    Color = ColorSequence.new{
      ColorSequenceKeypoint.new(0.00, Color3.fromRGB(15, 15, 15)),
      ColorSequenceKeypoint.new(0.55, Color3.fromRGB(28, 12, 12)),
      ColorSequenceKeypoint.new(1.00, Custom.ColorRGB),
    },
  ...
  function Tabs:CreateTab(Config)
    ...
    -- ensure ScrollTab and Tab don't clip the side indicator/glow
    Tab.ClipsDescendants = false
    ScrollTab.ClipsDescendants = false

    if CountTab == 0 then
      LayersPageLayout:JumpToIndex(0)
      NameTab.Text = _Name

      -- create ONE sidebar-level indicator (parented to ScrollTab so it can travel across all tabs)
      if not ScrollTab:FindFirstChild("ChooseFrame") then
        ...
        -- animated shimmer gradient on the active indicator
        local IndicatorGrad = Custom:Create("UIGradient", {
          Rotation = 90,
          Color = ColorSequence.new{
            ColorSequenceKeypoint.new(0.00, Color3.fromRGB(255, 90, 90)),
            ColorSequenceKeypoint.new(0.50, Custom.ColorRGB),
            ColorSequenceKeypoint.new(1.00, Color3.fromRGB(180, 0, 0)),
          },
        }, ChooseFrame)

        task.spawn(function()
          while ChooseFrame.Parent do
            local t = TweenService:Create(IndicatorGrad, TweenInfo.new(1.6, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, -1, true), {Offset = Vector2.new(0, 1)})
            t:Play()
            task.wait(3.2)
          end
        end)
    ...
    TabButton.Activated:Connect(function()
      ...
      local FrameChoose = ScrollTab:FindFirstChild("ChooseFrame")
      ...
      if FrameChoose and Tab.LayoutOrder ~= LayersPageLayout.CurrentPage.LayoutOrder then
        ...
        local _TabT = TweenService:Create(Tab, TweenInfo.new(0.45, Enum.EasingStyle.Back, Enum.EasingDirection.InOut), {BackgroundTransparency = 0.92})

        -- compute target Y from REAL layout position (handles padding / first-tab gap correctly,
        -- and the indicator is parented to ScrollTab so it never clips inside a Tab frame)
        local targetY = Tab.AbsolutePosition.Y - ScrollTab.AbsolutePosition.Y + math.max(0, (Tab.AbsoluteSize.Y - 14) // 2)
        local _FTween = TweenService:Create(FrameChoose, TweenInfo.new(0.45, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut), { Position = UDim2.new(0, 2, 0, targetY) })
    ...
    function Sections:AddSection(Title, OpenSection)
      ...
      Custom:Create("UIGradient", {
        Color = ColorSequence.new{
          ...
          ColorSequenceKeypoint.new(0.25, Color3.fromRGB(120, 20, 20)),
          ColorSequenceKeypoint.new(0.50, Custom.ColorRGB),
          ColorSequenceKeypoint.new(0.75, Color3.fromRGB(120, 20, 20)),
